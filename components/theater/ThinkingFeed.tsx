  ...
  return (
    <ConsolePanel className="overflow-hidden">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between px-4 py-3 text-sm text-muted transition-colors hover:text-ink"
      >
      ...
      {open && (
        <ul className="max-h-48 divide-y divide-line overflow-y-auto border-t border-line">
          {lines.map((line, i) => (
            <li key={i} className="animate-log-in px-4 py-2 font-mono text-[11px] leading-relaxed text-muted">
              {line}
            </li>
          ))}
        </ul>
