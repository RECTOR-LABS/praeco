...
export interface InboundNegotiation { negotiationId: string; status: string; requirements: string }
...
interface SdkClient {
  listNegotiations(opts: { role: string; status?: string }): Promise<Array<{ negotiationId: string; status: string; requirements?: string }>>;
...
export class AgentClientProvider implements CapProvider {
  ...
  async listInboundNegotiations(): Promise<InboundNegotiation[]> {
    ...
    return ns.map((n) => ({ negotiationId: n.negotiationId, status: n.status, requirements: n.requirements ?? "" }));
  }
